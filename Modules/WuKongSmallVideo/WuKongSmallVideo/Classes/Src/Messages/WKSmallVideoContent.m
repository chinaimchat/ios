...
@property(nonatomic,copy) NSString *videoSourceFilePath;
@end
...
+(WKSmallVideoContent*) smallVideoContentWithVideoURL:(NSString*)videoFilePath coverData:(NSData*)coverData second:(NSInteger)second {
    WKSmallVideoContent *smallVideoContent = [WKSmallVideoContent new];
    smallVideoContent.videoSourceFilePath = videoFilePath;
    smallVideoContent.coverData = coverData;
    smallVideoContent.second = second;
    NSDictionary *attrs = [[NSFileManager defaultManager] attributesOfItemAtPath:videoFilePath error:nil];
    smallVideoContent.size = [attrs fileSize];
    UIImage *coverImage = [UIImage imageWithData:coverData];
    if(coverImage) {
        smallVideoContent.width = coverImage.size.width;
        smallVideoContent.height = coverImage.size.height;
...
- (void)writeDataToLocalPath {
    ...
    if(self.videoSourceFilePath) {
        NSError *error;
        if(![fm moveItemAtPath:self.videoSourceFilePath toPath:self.localPath error:&error]) {
            if(![fm copyItemAtPath:self.videoSourceFilePath toPath:self.localPath error:&error]) {
                WKLogError(@"视频文件写入失败: %@", error);
            }
        }
