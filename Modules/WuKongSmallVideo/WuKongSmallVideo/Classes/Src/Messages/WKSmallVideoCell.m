...
static NSMutableSet<NSString *> *wkVideoCoverDiskInFlight(void) {
...
//    self.trailingView.timeLbl.textColor = [UIColor whiteColor];
    ...
    if([[NSFileManager defaultManager] fileExistsAtPath:[smallVideoContent coverLocalPath]]) {
        ...
        NSURL *coverURL = [[WKApp shared] getImageFullUrl:smallVideoContent.cover];
        if (coverURL) {
            __weak typeof(self) weakSelf = self;
            UIImage *ph = [self imageName:@"DefaultVideo"];
            [self.coverImgView sd_setImageWithURL:coverURL
                                 placeholderImage:ph
                                          options:SDWebImageAllowInvalidSSLCertificates | SDWebImageLowPriority
                                          context:@{ SDWebImageContextStoreCacheType: @(SDImageCacheTypeAll) }
                                         progress:nil
                                        completed:^(UIImage * _Nullable image, NSError * _Nullable error, SDImageCacheType cacheType, NSURL * _Nullable imageURL) {
                if (!smallVideoContent.flame && image && smallVideoContent.message && smallVideoContent.cover.length > 0) {
                    [weakSelf wk_persistVideoCoverImage:image forContent:smallVideoContent];
                }
            }];
        } else {
            self.coverImgView.image = [self imageName:@"DefaultVideo"];
        }
    }
...
/// 与 Android 视频封面 Glide 缓存 + 本地封面路径一致：封面落盘后下次直接 file:// 加载。
- (void)wk_persistVideoCoverImage:(UIImage *)image forContent:(WKSmallVideoContent *)content {
    ...
    NSMutableSet *inFlight = wkVideoCoverDiskInFlight();
    ...
    NSString *path = [content coverLocalPath];
