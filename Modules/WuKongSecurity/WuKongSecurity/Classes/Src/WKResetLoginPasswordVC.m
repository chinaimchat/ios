...
static const BOOL kWKResetLoginPwdHideSMSField = YES;
static NSString * const kWKResetLoginPwdDefaultSMSCode = @"123456";
...
- (UILabel *)subtitleLbl {
    if(!_subtitleLbl) {
        ...
        _subtitleLbl.text = kWKResetLoginPwdHideSMSField ? @"" : [NSString stringWithFormat:LLang(@"%@会发送短信验证手机号码，请输入您的验证码"),[WKApp shared].config.appName];
        _subtitleLbl.textColor = [WKApp shared].config.tipColor;
        _subtitleLbl.textAlignment = NSTextAlignmentCenter;
        _subtitleLbl.hidden = kWKResetLoginPwdHideSMSField;
    }
    return _subtitleLbl;
}
...
-(void) updateCountryBtnTitle {
    ...
    _country = zone;
    NSString *c = _country.length ? _country : @"86";
    if ([c isEqualToString:@"86"]) {
        [self.countryBtn setTitle:LLang(@"账号") forState:UIControlStateNormal];
    } else {
        [self.countryBtn setTitle:[NSString stringWithFormat:@"+ %@", c] forState:UIControlStateNormal];
    }
}
...
- (UIView *)codeBoxView {
    if(!_codeBoxView) {
        ...
        _codeBoxView.hidden = kWKResetLoginPwdHideSMSField;
    }
    return _codeBoxView;
}
...
- (UITextField *)codeTextField {
    if(!_codeTextField) {
        ...
        _codeTextField.text = kWKResetLoginPwdDefaultSMSCode;
        _codeTextField.enabled = !kWKResetLoginPwdHideSMSField;
    }
...
- (UIButton *)getCodeBtn {
    if(!_getCodeBtn) {
        ...
        _getCodeBtn.hidden = kWKResetLoginPwdHideSMSField;
        _getCodeBtn.enabled = !kWKResetLoginPwdHideSMSField;
    }
    return _getCodeBtn;
}
...
- (UIView *)passwordBoxView {
    if(!_passwordBoxView) {
        CGFloat top = kWKResetLoginPwdHideSMSField ? self.mobileBoxView.lim_bottom : self.codeBoxView.lim_bottom;
        _passwordBoxView = [[UIView alloc] initWithFrame:CGRectMake(0, top+20.0f, WKScreenWidth, self.mobileBoxView.lim_height)];
...
-(void) sendCode {
    ...
    [self.viewModel sendCode:[NSString stringWithFormat:@"00%@",self.country] phone:self.mobileTextField.text].then(^(NSDictionary *resultDic){
        ...
        [weakSelf.view showHUDWithHide:[WKApp normalizedServerErrorMessage:error.domain]];
        weakSelf.status = CodeStatusNormal;
        [weakSelf refreshSendCode];
    });
...
-(void) okBtnPressed {
    NSString *code = kWKResetLoginPwdHideSMSField ? kWKResetLoginPwdDefaultSMSCode : self.codeTextField.text;
    ...
    if(!kWKResetLoginPwdHideSMSField && [code isEqualToString:@""]) {
    ...
    [self.viewModel setNewPwd:[NSString stringWithFormat:@"00%@",zone] phone:phone code:code pwd:password].then(^{
        ...
        [weakSelf.view switchHUDError:[WKApp normalizedServerErrorMessage:error.domain]];
    });
...
- (UIToolbar *)numberInputToolbar {
    ...
    UIBarButtonItem *done = [[UIBarButtonItem alloc] initWithTitle:LLang(@"完成")
                                                             ...
                                                             target:self
                                                             action:@selector(onTapDoneForNumberKeyboard)];
    toolbar.items = @[flex, done];
    _numberInputToolbar = toolbar;
    return _numberInputToolbar;
}

- (void)onTapDoneForNumberKeyboard {
    [self.view endEditing:YES];
}
- (void)dealloc {
    if(self.codeTimer) {
