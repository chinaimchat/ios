...
-(void) initTask {
    ...
    } if( self.message.contentType == WK_SMALLVIDEO || self.message.contentType == WK_VIDEO || self.message.contentType == WK_FILE) {
        realLocalPath = media.localPath;
    }
    ...
    if(self.message.contentType == WK_SMALLVIDEO || self.message.contentType == WK_VIDEO) { // 视频类：直存正文视频文件（与 Android WK_VIDEO 同号时与小视频共用分支）
