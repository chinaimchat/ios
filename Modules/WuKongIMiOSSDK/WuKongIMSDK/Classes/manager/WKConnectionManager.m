...
/// 将 host:port 拆成主机与端口；用最后一个冒号分割，避免 tcp://host:port 或多段冒号时误解析。
static void WKParseImHostPort(NSString *rawAddr, NSString * __autoreleasing *outHost, uint16_t *outPort) {
    *outHost = nil;
    *outPort = 0;
    if (!rawAddr.length) {
        return;
    }
    NSString *s = [rawAddr stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if ([s.lowercaseString hasPrefix:@"tcp://"]) {
        s = [s substringFromIndex:6];
    }
    NSRange r = [s rangeOfString:@":" options:NSBackwardsSearch];
    if (r.location == NSNotFound || r.location == 0 || r.location + 1 >= s.length) {
        return;
    }
    NSString *host = [[s substringToIndex:r.location] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    NSString *portStr = [[s substringFromIndex:r.location + 1] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
...
-(void) loopGetAddrToConnect {
    ...
    if(self.getConnectAddr) {
        ...
        self.getConnectAddr(^(NSString * _Nonnull addr) {
            ...
            NSString *host = nil;
            uint16_t port = 0;
            WKParseImHostPort(addr, &host, &port);
            if (!host.length || port == 0) {
                if ([WKSDK shared].isDebug) {
                    NSLog(@"IM 地址无效，无法建立 socket：%@", addr);
                }
                dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1.0 * NSEC_PER_SEC)), dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
                    [weakSelf loopGetAddrToConnect];
                });
            ...
            [weakSelf connectSocket:host port:port];
        });
    }else {
