...
typedef enum : NSUInteger {
    ...
    // 1- 999 为业务消息
    WK_SMALLVIDEO = 5, // 小视频（与 SDK `WK_VIDEO` / Android `WKMsgContentType.WK_VIDEO` 同号 5）
    WK_LOCATION = 6, // 位置消息
    WK_CARD = 7, // 名片
    WK_FILE = 8, // 文件
    WK_REDPACKET = 9, // 红包
    ...
    /// 钱包余额 IM 同步（与 Android {@code WKContentType.WK_WALLET_BALANCE_SYNC} 一致）
    WK_WALLET_BALANCE_SYNC = 1020,
