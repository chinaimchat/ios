        ...
        switch (type) {
            case PHAssetMediaTypeImage:
                if (path) {
                    ...
                    if(!image && ([ext isEqualToString:@"heic"] || [ext isEqualToString:@"heif"])) {
                        if (@available(iOS 13.0, *)) {
                            image = [[SDImageHEICCoder sharedCoder] decodedImageWithData:[[NSData alloc] initWithContentsOfFile:path] options:@{SDImageCoderEncodeCompressionQuality:@(0.9)}];
                        }
                    }
                    if(!image) {
                        image = img;
                    }
                    if(image && weakSelf.completeBlock) {
                        weakSelf.completeBlock(image);
                    }
                }else {
                    if(img && weakSelf.completeBlock) {
