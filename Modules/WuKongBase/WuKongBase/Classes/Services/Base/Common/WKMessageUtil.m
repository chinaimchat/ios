...
+(WKMessage*) toMessage:(NSDictionary*)messageDict {
    ...
    BOOL hasRedDotField = NO;
    if(headerDict) {
        hasRedDotField = headerDict[@"red_dot"] != nil;
        message.header.showUnread = hasRedDotField ? [headerDict[@"red_dot"] integerValue] : 0;
    ...
    // 某些欢迎/系统消息 payload header 里偶发不带 red_dot，导致「有消息但不计未读/不提醒」。
    // 这里仅对系统号做兜底，避免影响普通会话的服务端控制策略。
    if (!hasRedDotField
        && message.contentType != WK_CMD
        && message.fromUid.length > 0
        && [message.fromUid isEqualToString:[WKApp shared].config.systemUID]) {
