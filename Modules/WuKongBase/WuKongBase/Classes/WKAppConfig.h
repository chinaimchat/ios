...
/// 会话列表等首页导航标题（可与 appName 不同）
@property(nonatomic,copy) NSString *conversationHomeTitle;
...
/// 客服热线接口使用的应用标识，与 Android {@code WKBaseApplication#appID} / {@code WKCustomerServiceApplication#customerServiceAppId} 一致；未设置时由业务层回退 bundle 或默认值。
@property(nonatomic,copy,nullable) NSString *customerServiceAppId;
...
@property(nonatomic,assign) BOOL inviteCodeSystemOn; // 邀请码系统总开关（控制注册是否必须填邀请码）
...
/// common/appconfig：为 0 时不展示对方 Web/手机/PC 等分端在线文案（仅保留是否在线）；默认 1
@property(nonatomic,assign) NSInteger showDeviceOnlineOn;
