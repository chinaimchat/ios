...
#if __has_include(<BuglyPro/Bugly.h>)
#import <BuglyPro/Bugly.h>
#elif __has_include(<Bugly/Bugly.h>)
...
/// 与 Android {@code WalletChatRouter.openOfficialCustomerService(activity)} 一致：在承载 BottomSheet 的宿主 Activity 的 Nav 上打开会话，而不是在 Sheet 自身上 push。
static UINavigationController *WKNavigationControllerForCustomerService(id contextParam) {
    UIViewController *vc = [contextParam isKindOfClass:[UIViewController class]] ? (UIViewController *)contextParam : nil;
    UIViewController *walker = vc;
    while (walker) {
        UINavigationController *nav = walker.navigationController;
        if (nav) {
            return nav;
        }
        walker = walker.presentingViewController;
    }
    return [[WKNavigationManager shared] topViewController].navigationController;
}

static UIView *WKViewForCustomerServiceFeedback(id contextParam) {
    if ([contextParam isKindOfClass:[UIViewController class]]) {
        return ((UIViewController *)contextParam).view;
    }
...
static NSString *WKResolvedIMConnectToken(WKLoginInfo *loginInfo) {
    NSString *imToken = [loginInfo.imToken stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if (imToken.length > 0) {
        return imToken;
    }
    NSString *token = [loginInfo.token stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if (token.length > 0) {
        loginInfo.imToken = token;
        [loginInfo save];
        WKLogInfo(@"IM token empty, fallback to login token for ws connect.");
        return token;
    }
    return @"";
}
...
static NSString *WKImTcpAddrFromAPIPayload(id payload) {
    ...
    id addrObj = root[@"tcp_addr"] ?: root[@"tcpAddr"];
    if (addrObj == nil || ([addrObj isKindOfClass:[NSString class]] && [(NSString *)addrObj length] == 0)) {
        id inner = root[@"data"];
        if ([inner isKindOfClass:[NSDictionary class]]) {
            NSDictionary *d = (NSDictionary *)inner;
            addrObj = d[@"tcp_addr"] ?: d[@"tcpAddr"];
        }
    }
    if (addrObj == nil || ([addrObj isKindOfClass:[NSString class]] && [(NSString *)addrObj length] == 0)) {
        id inner = root[@"result"];
        if ([inner isKindOfClass:[NSDictionary class]]) {
            NSDictionary *d = (NSDictionary *)inner;
            addrObj = d[@"tcp_addr"] ?: d[@"tcpAddr"];
        }
    ...
    if ([addrObj isKindOfClass:[NSNumber class]]) {
        addrObj = [(NSNumber *)addrObj stringValue];
    }
    if (![addrObj isKindOfClass:[NSString class]]) {
        return nil;
    }
    NSString *addr = [(NSString *)addrObj stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
    if ([addr.lowercaseString hasPrefix:@"tcp://"]) {
...
+ (NSString *)normalizedServerErrorMessage:(NSString *)message {
    if (message.length == 0) {
        return message;
    }
    NSString *lowerMsg = message.lowercaseString;
    if ([lowerMsg containsString:@"手机号不合法"]
        || [lowerMsg containsString:@"手机号码不合法"]
        || [lowerMsg containsString:@"illegal mobile phone number"]
        || [lowerMsg containsString:@"invalid mobile phone number"]
        || [lowerMsg containsString:@"phone number is invalid"]) {
        return LLang(@"账号输入有误，请检查后重试");
    }
    return message;
}
...
-(void) configSDWebImage {
    ...
    /// 与 Android Glide 磁盘策略类似：延长 SD 磁盘条目保留时间（默认约 7 天），减少会话图片重复下载。
    SDImageCache.sharedImageCache.config.maxDiskAge = 30 * 24 * 60 * 60;
...
-(void) traceConfig {
#if __has_include(<BuglyPro/Bugly.h>)
    // BuglyPro：使用 initWithAppId:appKey:、startWithConfig:、updateUserIdentifier:
    BuglyConfig *config = [[BuglyConfig alloc] initWithAppId:@"82f8dd98ff" appKey:@""];
    config.serverHostType = BuglyServerHostTypeBuglyPro;
#ifndef __OPTIMIZE__
    ...
    if ([WKApp shared].isLogined) {
        [Bugly updateUserIdentifier:[WKApp shared].loginInfo.uid];
    }
#elif __has_include(<Bugly/Bugly.h>)
...
- (void)application:(UIApplication *)application didReceiveRemoteNotification:(NSDictionary *)userInfo fetchCompletionHandler:(void (^)(UIBackgroundFetchResult))completionHandler {
    // 与 Android FCM 唤醒类似：带 content-available 的静默推送在后台拉起进程时，尽快尝试 IM 重连。
    // 服务端需在 aps 中配置 "content-available": 1，且勿滥用；各模块仍通过 completionHandler 上报结果（勿重复调用）。
    if ([WKApp shared].isLogined) {
        [self connectIMIfPossible];
    }
...
//    [self debugSetting];
    ...
    [[WKSDK shared].options setConnectInfoCallback:^WKConnectInfo * _Nonnull{
        ...
        connectInfo.token = WKResolvedIMConnectToken(weakSelf.loginInfo);
        return  connectInfo;
    }];
    ...
    if([WKApp shared].config.clusterOn) {
       [[WKSDK shared].connectionManager setGetConnectAddr:^(void (^ _Nonnull complete)(NSString * __nullable)) {
           [[WKAPIClient sharedClient] GET:[NSString stringWithFormat:@"users/%@/im",weakSelf.loginInfo.uid] parameters:nil].then(^(NSDictionary *addrDict){
               NSString *tcpAddr = WKImTcpAddrFromAPIPayload(addrDict);
               if (tcpAddr.length > 0) {
                   complete(tcpAddr);
               } else {
                   WKLogError(@"获取IM连接地址：响应中无可用 tcp_addr，payload=%@", addrDict);
                   complete(nil);
               }
...
-(void) appWillEnterForeground:(NSNotification*) notification {
    ...
    if ([WKApp shared].isLogined && [[WKSDK shared] connectionManager].connectStatus == WKDisconnected) {
        [self connectIMIfPossible];
    }
}
...
- (void)connectIMIfPossible {
    if (![WKApp shared].isLogined) {
        return;
    }
    NSString *uid = [[self.loginInfo.uid ?: @"" stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]] copy];
    NSString *token = WKResolvedIMConnectToken(self.loginInfo);
    if (uid.length == 0 || token.length == 0) {
        WKLogError(@"Skip ws connect because uid/token is empty. uid=%@ tokenLength=%lu", uid, (unsigned long)token.length);
        return;
    }
...
-(void) initPointMethods {
    ...
    /// 与 Android {@code WKCustomerServiceApplication} / {@code WalletChatRouter.openOfficialCustomerService} 一致：
    /// {@code POST hotline/visitor/topic/channel}，再打开会话。
    [self setMethod:@"show_customer_service" handler:^id _Nullable(id _Nonnull param) {
        // 与 Android {@code CustomerServiceModel#getChatInfo} 的 body {@code appid} 一致；优先 {@code WKAppConfig.customerServiceAppId}（AppDelegate 与 TSApplication 对齐）。
        NSString *appId = [WKApp shared].config.customerServiceAppId;
        if (!appId.length) {
            appId = [WKApp shared].config.bundleID;
        }
        if (!appId.length) {
        ...
        /// 与 {@link WKCustomerServiceManager#visitorTopicChannelGetOrCreate} 一致：body 与 {@code appid} 请求头同时携带，与联系人「专属客服」同源。
        [[WKAPIClient sharedClient] POST:@"hotline/visitor/topic/channel" parameters:@{ @"topic_id": @(0), @"appid": appId } headers:@{ @"appid": appId }].then(^(id responseObj) {
            dispatch_async(dispatch_get_main_queue(), ^{
                NSDictionary *root = [responseObj isKindOfClass:[NSDictionary class]] ? responseObj : nil;
                NSDictionary *data = root[@"data"];
                if (![data isKindOfClass:[NSDictionary class]]) {
                    data = root;
                }
                id rawId = data[@"channel_id"];
                NSString *channelId = nil;
                if ([rawId isKindOfClass:[NSString class]]) {
                    channelId = [(NSString *)rawId stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                } else if (rawId != nil && rawId != [NSNull null]) {
                    channelId = [[NSString stringWithFormat:@"%@", rawId] stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
                }
                id typeVal = data[@"channel_type"];
                uint8_t channelType = WK_PERSON;
                if ([typeVal isKindOfClass:[NSNumber class]]) {
                    channelType = (uint8_t)[(NSNumber *)typeVal unsignedIntValue];
                } else if ([typeVal isKindOfClass:[NSString class]] && [(NSString *)typeVal length] > 0) {
                    channelType = (uint8_t)[(NSString *)typeVal intValue];
                }
                if (!channelId.length) {
                    NSString *msg = root[@"msg"];
                    if (![msg isKindOfClass:[NSString class]] || ![(NSString *)msg length]) {
                        msg = LLangW(@"无法打开客服会话", weakSelf);
                    }
                    [WKViewForCustomerServiceFeedback(param) showHUDWithHide:msg];
                    return;
                }
                WKChannel *channel = [[WKChannel alloc] initWith:channelId channelType:channelType];
                UINavigationController *nav = WKNavigationControllerForCustomerService(param);
                WKConversationVC *conversationVC = [WKConversationVC new];
                conversationVC.channel = channel;
                if (nav) {
                    [nav pushViewController:conversationVC animated:YES];
                } else {
                    [[WKNavigationManager shared] pushViewController:conversationVC animated:YES];
                }
            });
    ...
    // emoji：与 Android ChatPanelManager 一致插在工具栏最前（sort 取较小值，WKFuncGroupView 按 sort 升序排，@ 见下）
    [self setMethod:WKPOINT_CATEGORY_PANELFUNCITEM_EMOJI handler:^id _Nullable(id  _Nonnull param) {
        ...
        item.sort = 1000;
        return item;
    } category:WKPOINT_CATEGORY_PANELFUNCITEM sort:100];
    
    // voice / image：与 Android WKUIKitApplication 工具栏 sort 一致（降序展示为 相册99、语音97），横向顺序为 emoji 后「图片 → 语音」。
    [self setMethod:WKPOINT_CATEGORY_PANELFUNCITEM_VOICE handler:^id _Nullable(id  _Nonnull param) {
        ...
        item.sort = 3000;
        return item;
    } category:WKPOINT_CATEGORY_PANELFUNCITEM sort:97];
    ...
    [self setMethod:WKPOINT_CATEGORY_PANELFUNCITEM_IMAGE handler:^id _Nullable(id  _Nonnull param) {
        ...
        item.sort = 2000;
        return item;
    } category:WKPOINT_CATEGORY_PANELFUNCITEM sort:99];
    ...
    [self setMethod:WKPOINT_CATEGORY_PANELFUNCITEM_MENTION handler:^id _Nullable(id  _Nonnull param) {
        ...
        return item;
    } category:WKPOINT_CATEGORY_PANELFUNCITEM sort:96];
    ...
    [self setMethod:WKPOINT_CATEGORY_PANELFUNCITEM_CARD handler:^id _Nullable(id  _Nonnull param) {
        ...
        item.sort = 5000;
        return item;
    } category:WKPOINT_CATEGORY_PANELFUNCITEM sort:95];
    

    
    // more：Android wkChatToolBar + "_more" sort=40，固定排在工具栏末尾（WKFuncGroupView 对 apm.wukong.more 特判）
    [self setMethod:WKPOINT_CATEGORY_PANELFUNCITEM_MORE handler:^id _Nullable(id  _Nonnull param) {
        WKPanelMoreFuncItem *item = [[WKPanelMoreFuncItem alloc] init];
        item.sort = 40;
        return item;
    } category:WKPOINT_CATEGORY_PANELFUNCITEM sort:40];
...
//    } category:WKPOINT_CATEGORY_PANELCONTENT sort:1000];
    ...
    // 对齐 Android WKUIKitApplication chatFunction：图片(100)、名片(95)，与 FaceManger 宫格一致
    [self setMethod:@"chat_function_chooseImg" handler:^id _Nullable(id  _Nonnull param) {
        NSDictionary *dict = param;
        id<WKConversationContext> ctx = dict[@"context"];
        if (!ctx) {
            return nil;
        }
        UIImage *img = [weakSelf loadImage:@"Conversation/Toolbar/ImageNormal" moduleID:@"WuKongBase"];
        return [WKMoreItemModel initWithImage:img title:LLang(@"图片") onClick:^(id<WKConversationContext> conversationContext) {
            [conversationContext endEditing];
            [[WKMoreItemClickEvent shared] onPhotoItemPressed:conversationContext];
        }];
    } category:WKPOINT_CATEGORY_PANELMORE_ITEMS sort:100];

    [self setMethod:@"chat_function_chooseCard" handler:^id _Nullable(id  _Nonnull param) {
        NSDictionary *dict = param;
        id<WKConversationContext> ctx = dict[@"context"];
        if (!ctx) {
            return nil;
        }
        UIImage *img = [weakSelf loadImage:@"Conversation/Toolbar/CardNormal" moduleID:@"WuKongBase"];
        return [WKMoreItemModel initWithImage:img title:LLang(@"名片") onClick:^(id<WKConversationContext> conversationContext) {
            [conversationContext endEditing];
            [WKPanelCardFuncItem presentCardPickerForConversationContext:conversationContext toolbarButton:nil];
        }];
    } category:WKPOINT_CATEGORY_PANELMORE_ITEMS sort:95];
...
//        }
    ...
    // ---------- 我的  ----------
    // 与 Android `personal_center_*` 同序：wallet(1000) 在 WuKongWallet；web_login(999)、new_msg_notice(3)、currency/通用(2) 见下；其余扩展项排在后。
    [self setMethod:WKPOINT_ME_WEB handler:^id _Nullable(id  _Nonnull param) {
        return [WKMeItem initWithTitle:LLangW(@"电脑端登录",weakSelf) icon:[weakSelf imageName:@"Me/Index/IconPC"] onClick:^{
            [[WKNavigationManager shared] pushViewController:[WKWebClientInfoVC new] animated:YES];
        }];
    } category:WKPOINT_CATEGORY_ME sort:19900];
    // 新消息通知
    ...
    } category:WKPOINT_CATEGORY_ME sort:19820];
    
    // 我的邀请码（在「新消息通知」之上）
    [self setMethod:WKPOINT_ME_INVITE handler:^id _Nullable(id  _Nonnull param) {
        // 业务要求：iOS「我的」中固定隐藏「我的邀请码」入口
        return nil;
    } category:WKPOINT_CATEGORY_ME sort:19840];
    ...
    } category:WKPOINT_CATEGORY_ME sort:19700];
