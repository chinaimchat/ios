...
-(void) registerSections {
    ...
    [[WKApp shared] setMethod:@"channelsetting.input_toolbar_customize" handler:^id _Nullable(id  _Nonnull param) {
        id<WKConversationContext> ctx = param[@"context"];
        if (!ctx) {
            return nil;
        }
        return @{
            ...
            @"items": @[
                @{
                    @"class": WKLabelItemModel.class,
                    @"label": LLang(@"自定义聊天工具栏"),
                    @"showBottomLine": @(NO),
                    @"showTopLine": @(NO),
                    @"onClick": ^{
                        WKFuncGroupEditVC *vc = [[WKFuncGroupEditVC alloc] init];
                        vc.conversationContext = ctx;
                        [[WKNavigationManager shared] pushViewController:vc animated:YES];
                    },
                },
            ],
        };
    } category:WKPOINT_CATEGORY_CHANNELSETTING sort:89575];
