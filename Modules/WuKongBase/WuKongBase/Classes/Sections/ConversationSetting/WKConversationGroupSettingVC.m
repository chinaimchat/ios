...
- (void)cropGroupAvatar:(UIImage *)avatarImg {
...
- (void)cropViewController:(nonnull TOCropViewController *)cropViewController
    ...
    NSString *uploadPath = [NSString stringWithFormat:@"groups/%@/avatar", self.channel.channelId];
    [[WKAPIClient sharedClient] fileUpload:uploadPath data:data progress:^(NSProgress * _Nonnull progress) {
        ...
        if (error) {
            ...
            [WKApp.shared notifyChannelAvatarUpdate:[WKChannel channelID:weakSelf.channel.channelId channelType:WK_GROUP]];
        }
    }];
}
