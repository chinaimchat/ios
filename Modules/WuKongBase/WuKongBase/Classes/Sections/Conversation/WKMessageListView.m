...
#import "../../Services/Base/WKChatMediaPrefetcher.h"
...
- (void)wk_scheduleInitialVideoPrefetchIfNeeded:(BOOL)firstLoad;
...
-(void) handleLoadMessages:(BOOL)animation firstLoad:(BOOL)firstLoad hasMore:(bool)hasMore complete:(void(^)(void))complete {
    ...
    if(!self.keepPosition) {
        ...
        if(!indexPath) {
            [self scrollToBottom:animation];
            [self wk_scheduleInitialVideoPrefetchIfNeeded:firstLoad];
            return;
        }
    ...
    if(firstLoad) {
        ...
        [self wk_scheduleInitialVideoPrefetchIfNeeded:YES];
    }
}

- (void)wk_scheduleInitialVideoPrefetchIfNeeded:(BOOL)firstLoad {
    if (!firstLoad || !self.tableView || !self.dataProvider) {
        return;
    }
    __weak typeof(self) weakSelf = self;
    dispatch_async(dispatch_get_main_queue(), ^{
        WKMessageListView *selfStrong = weakSelf;
        if (!selfStrong) {
            return;
        }
        [selfStrong.tableView layoutIfNeeded];
        [WKChatMediaPrefetcher schedulePrefetchSmallVideosNearVisibleRowsInTableView:selfStrong.tableView dataProvider:selfStrong.dataProvider];
    });
}
...
-(void) endScroll:(UIScrollView*)scrollView {
    ...
    [WKChatMediaPrefetcher prefetchImageURLsForVisibleRowsInTableView:self.tableView dataProvider:self.dataProvider];
    [WKChatMediaPrefetcher schedulePrefetchSmallVideosNearVisibleRowsInTableView:self.tableView dataProvider:self.dataProvider];
}
