...
- (UIButton *)editBtn {
    if(!_editBtn) {
        ...
        [_editBtn sizeToFit];
        // 与 WKNavigationBar.backButton（左 15、宽约 44）错开，否则会盖住返回键导致无法返回
        CGFloat backTrailing = 15.0f + 44.0f + 8.0f;
        _editBtn.lim_left = backTrailing;
...
- (UIButton *)completeBtn {
    if(!_completeBtn) {
        ...
        _completeBtn.lim_left = self.view.lim_width - _completeBtn.lim_width - 15.0f;
...
-(void) dismissPressed {
    // 由会话设置 push 进入，需 pop；dismiss 仅适用于模态
