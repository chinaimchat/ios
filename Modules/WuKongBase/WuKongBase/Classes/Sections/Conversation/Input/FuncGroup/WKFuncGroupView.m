...
/// 横向滚动区右侧留白（「更多」已固定在最右槽位，此处仅收尾留白）。
static const CGFloat kWKFuncGroupScrollTrailingPadding = 12.0f;

/// 固定「更多」槽位宽度：与 `WKFuncGroupItemView` 在高度 h 下的宽度 (h+10) 一致，再加左右内边距。
static CGFloat WKFuncGroupTrailingMoreSlotWidth(CGFloat barHeight) {
    return MAX(0.0f, barHeight) + 10.0f + 12.0f;
}
...
@property(nonatomic,strong) UIView *trailingMoreContainer;
@property(nonatomic,strong,nullable) WKFuncGroupItemView *pinnedMoreItemView;
...
- (instancetype)initWithFrame:(CGRect)frame inputPanel:(WKConversationInputPanel*)inputPanel
{
    ...
    if (self) {
        ...
        [self addSubview:self.trailingMoreContainer];
        [self reloadData];
...
-(void) reloadData {
    ...
    [self.pinnedMoreItemView removeFromSuperview];
    self.pinnedMoreItemView = nil;

    NSMutableArray<WKPanelDefaultFuncItem *> *leadingItems = [NSMutableArray array];
    WKPanelDefaultFuncItem *moreFuncItem = nil;
    for (WKPanelDefaultFuncItem *funcItem in self.funcItems) {
        if ([[funcItem sid] isEqualToString:@"apm.wukong.more"]) {
            moreFuncItem = funcItem;
        } else {
            [leadingItems addObject:funcItem];
        }
    }

    WKPanelDefaultFuncItem *preFuncItem;
    for (WKPanelDefaultFuncItem *funcItem in leadingItems) {
        ...
        if (preFuncItem) {
            if (preFuncItem.type != funcItem.type) {
                showSplit = true;
            }
        }
        itemView.showSplit = showSplit;
    ...
    if (moreFuncItem) {
        WKFuncItemButton *btn = [moreFuncItem itemButton:self.inputPanel];
        __weak typeof(self) weakSelf = self;
        WKFuncGroupItemView *moreView = [[WKFuncGroupItemView alloc] initWithButton:btn scaleZoom:self.scaleZoom];
        [moreView setOnClick:^(WKFuncGroupItemView *item){
            [weakSelf unSelectedItems];
            item.selected = true;
            [item triggerClick];
        }];
        moreView.showSplit = YES;
        [self.trailingMoreContainer addSubview:moreView];
        self.pinnedMoreItemView = moreView;
        self.trailingMoreContainer.hidden = NO;
    } else {
        self.trailingMoreContainer.hidden = YES;
    }
...
-(void) unSelectedItems {
    ...
    if (self.pinnedMoreItemView) {
        self.pinnedMoreItemView.selected = NO;
    }
}
...
- (void)layoutSubviews {
    ...
    CGFloat trailW = self.pinnedMoreItemView ? WKFuncGroupTrailingMoreSlotWidth(barH) : 0.0f;
    CGFloat scrollW = MAX(0.0f, barW - trailW);
    self.scrollView.frame = CGRectMake(0.0f, 0.0f, scrollW, barH);
    self.trailingMoreContainer.frame = CGRectMake(scrollW, 0.0f, trailW, barH);
    self.trailingMoreContainer.backgroundColor = self.scrollView.backgroundColor;
    self.trailingMoreContainer.hidden = (trailW <= 0.0f);
    ...
    if(subviews.count>0) {
        ...
        [self.scrollView setContentSize:CGSizeMake(preView.lim_right + kWKFuncGroupScrollTrailingPadding, self.scrollView.lim_height)];
        
    } else {
        [self.scrollView setContentSize:CGSizeMake(scrollW, self.scrollView.lim_height)];
    }

    if (self.pinnedMoreItemView) {
        [self.pinnedMoreItemView layoutIfNeeded];
        self.pinnedMoreItemView.changeToBig = self.startScroll;
        self.pinnedMoreItemView.lim_centerX_parent = self.trailingMoreContainer;
        self.pinnedMoreItemView.lim_centerY_parent = self.trailingMoreContainer;
    }
...
- (UIView *)trailingMoreContainer {
    if (!_trailingMoreContainer) {
        _trailingMoreContainer = [[UIView alloc] init];
        _trailingMoreContainer.backgroundColor = self.scrollView.backgroundColor;
        _trailingMoreContainer.hidden = YES;
    }
    return _trailingMoreContainer;
}
