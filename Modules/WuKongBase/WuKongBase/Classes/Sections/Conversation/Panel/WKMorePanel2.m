...
// 与 Android FaceManger.getFunctionView + FunctionViewPageAdapter 对齐：4 列、每页 8 格（2 行×4 列），高度按面板均分。
...
// Android item_function_viewpage_layout：RecyclerView marginTop/Bottom 20dp；panel_function_layout 底部指示条约 23dp 区。
#define contentInsets  UIEdgeInsetsMake(20.0f, 10.0f, 36.0f, 10.0f)
...
-(void) layoutItems {
    ...
    self.pageControl.lim_top = self.lim_height - bottomSafeArea - 20.0f; // 对齐 Android dropRecyclerView layout_marginBottom 20dp
...
- (UIImageView *)iconImgView {
    if(!_iconImgView) {
        // Android item_function_layout functionIv 40dp
...
- (void)layoutSubviews {
    ...
    // Android item_function_layout：functionNameTv layout_marginTop 10dp（图标与标题间距）
