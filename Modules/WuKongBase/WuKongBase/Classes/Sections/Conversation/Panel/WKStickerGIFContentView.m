...
-(void) requestHotGif {
    ...
    [[WKAPIClient sharedClient] GET:@"sticker/user/sticker" parameters:@{@"category":keyword}].then(^(id responseObj) {
        NSDictionary *dic = nil;
        if ([responseObj isKindOfClass:[NSDictionary class]]) {
            id data = ((NSDictionary *)responseObj)[@"data"];
            dic = [data isKindOfClass:[NSDictionary class]] ? data : (NSDictionary *)responseObj;
        }
        if (!dic) {
            return;
        }
        WKStickerPackage *stickerPackage = (WKStickerPackage *)[WKStickerPackage fromMap:dic type:ModelMapTypeAPI];
