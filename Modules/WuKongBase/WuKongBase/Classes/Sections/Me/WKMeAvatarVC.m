...
-(void) moreBtnPress {
    ...
    [actionSheet addItem:[WKActionSheetButtonItem2 initWithTitle:LLang(@"从手机相册选择") onClick:^{
        [[WKPhotoBrowser shared] showPhotoLibraryWithSender:weakSelf selectCompressImageBlock:^(NSArray<NSData *> * _Nonnull images, NSArray<PHAsset *> * _Nonnull assets, BOOL isOriginal) {
            ...
            if (!image) {
                [weakSelf.view showHUDWithHide:LLang(@"图片处理失败")];
                return;
            }
...
didCropToImage:(nonnull UIImage *)image withRect:(CGRect)cropRect
    ...
    if(!data || data.length == 0) {
        [self.view showHUDWithHide:LLang(@"图片处理失败")];
        return;
    }
    
    __weak typeof(self) weakSelf = self;
    [self.view showHUD:LLang(@"上传中")];
    [[WKAPIClient sharedClient] fileUpload:@"users/{uid}/avatar" data:data fileName:@"avatar.jpg" progress:^(NSProgress * _Nonnull progress) {
