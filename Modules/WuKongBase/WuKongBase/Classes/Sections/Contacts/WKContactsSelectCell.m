...
/// 群成员库里的 member_avatar 常为空或相对路径；与聊天列表一致用 uid 拼头像地址。
static NSString *WKContactsSelectResolvedAvatarURL(WKContactsSelect *m) {
    if (!m) {
        return @"";
    }
    NSString *raw = m.avatar ?: @"";
    ...
    if (trimmed.length > 0) {
        ...
        NSString *full = [WKAvatarUtil getFullAvatarWIthPath:trimmed];
        if (full.length > 0) {
            return full;
        }
    }
    if (m.uid.length > 0) {
        NSString *byUid = [WKAvatarUtil getAvatar:m.uid];
        return byUid ?: @"";
    }
    return @"";
}
...
-(void) refreshWithModel:(id)cellModel{
    ...
    self.avatarImgView.url = WKContactsSelectResolvedAvatarURL(_contactSelectModel);
