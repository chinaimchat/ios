...
- (void)showFriendRequestNotificationIfNeed:(WKFriendRequestDBModel *)model {
    if (!model) {
        return;
    }
    if (![WKMySettingManager shared].newMsgNotice) {
        return;
    }
    ...
    NSString *title = model.name.length > 0 ? model.name : LLang(@"好友申请");
    NSString *body = LLang(@"请求添加你为好友");
    ...
    if (@available(iOS 10.0, *)) {
        ...
        NSString *identifier = [NSString stringWithFormat:@"friend_request_%@_%@", model.uid ?: @"", model.token ?: @""];
        UNNotificationRequest *request = [UNNotificationRequest requestWithIdentifier:identifier content:notifContent trigger:nil];
...
-(void) handleFriendRequest:(NSDictionary*)param {
    if(param) {
        ...
        if(isNewRequest) {
            [self showFriendRequestNotificationIfNeed:requestModel];
