...
- (void)presentLimitedLibraryPickerIfPossible:(UIViewController *)presenter {
    if (!presenter) {
        return;
    }
    if (@available(iOS 14.0, *)) {
        PHPhotoLibrary *library = [PHPhotoLibrary sharedPhotoLibrary];
        SEL selector = NSSelectorFromString(@"presentLimitedLibraryPickerFromViewController:");
        if (![library respondsToSelector:selector]) {
            return;
        }
        NSMethodSignature *signature = [library methodSignatureForSelector:selector];
        if (!signature) {
            return;
        }
        NSInvocation *invocation = [NSInvocation invocationWithMethodSignature:signature];
        [invocation setTarget:library];
        [invocation setSelector:selector];
        UIViewController *arg = presenter;
...
- (void)fetchPhotoFromLibrary:(WKLibraryFetchResult)result cancel:(void (^)(void))cancel
{
    ...
    [self setUpPhotoLibrary:^(WKMediaPickerController * _Nullable picker) {
        if (picker && weakSelf) {
            ...
            UIViewController *presenter = [LBXPermissionSetting currentTopViewController];
            if (!presenter) {
                presenter = [[WKNavigationManager shared] topViewController];
            }
            if (presenter) {
            ...
            case PHAssetMediaTypeImage:{
                if (path) {
                    ...
                    if(isSelectOriginalPhoto) {
                        ...
                        if ([pathExt isEqualToString:@"heic"] || [pathExt isEqualToString:@"heif"]){
                            UIImage *originImage = [UIImage imageWithContentsOfFile:path];
                            if(!originImage) {
                                if (@available(iOS 13.0, *)) {
                                    originImage = [[SDImageHEICCoder sharedCoder] decodedImageWithData:imageData options:@{SDImageCoderEncodeCompressionQuality:@(0.9)}];
                                }
                            }
                            if(originImage) {
                                NSData *jpegData = UIImageJPEGRepresentation(originImage, 0.9);
                                result(jpegData,path,isSelectOriginalPhoto,type,left);
                            } else {
...
- (void)fetchMediaFromCamera:(WKCameraFetchResult)result
{
    if ([self initCamera]) {
        ...
        [presenter presentViewController:self.imagePicker animated:YES completion:nil];
#endif
    }
}
...
- (void)setUpPhotoLibrary:(void(^)(WKMediaPickerController * _Nullable picker)) handler
{
    ...
    [PHPhotoLibrary requestAuthorization:^(PHAuthorizationStatus status){
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            BOOL canReadLibrary = (status == PHAuthorizationStatusAuthorized);
            if (@available(iOS 14.0, *)) {
                ...
                if (@available(iOS 14.0, *)) {
                    if (status == PHAuthorizationStatusLimited) {
                        ...
                        if (assets.count == 0) {
                            ...
                            if (presenter) {
                                [weakSelf presentLimitedLibraryPickerIfPossible:presenter];
                            }
                            if(handler) handler(nil);
                            return;
                        }
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeMovie]) {
        ...
        dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
            ...
            [weakSelf exportAssetWithFallback:asset completion:^(NSString *outputPath) {
                dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)requestAsset:(PHAsset *)asset  handler:(void(^)(NSString *,PHAssetMediaType)) handler
{
    if (asset.mediaType == PHAssetMediaTypeVideo) {
        ...
        [[PHImageManager defaultManager] requestExportSessionForVideo:asset options:options exportPreset:AVAssetExportPreset1280x720 resultHandler:^(AVAssetExportSession * _Nullable exportSession, NSDictionary * _Nullable info) {
            ...
            [exportSession exportAsynchronouslyWithCompletionHandler:^(void)
             {
                 dispatch_async(dispatch_get_main_queue(), ^{
                     ...
                     else
                     {
                         ...
                         [self requestAssetPassthrough:asset handler:handler];
                     }
                 });
             }];
        }];
...
- (void)requestAssetPassthrough:(PHAsset *)asset handler:(void(^)(NSString *,PHAssetMediaType)) handler
{
    PHVideoRequestOptions *options = [[PHVideoRequestOptions alloc] init];
    options.version = PHVideoRequestOptionsVersionCurrent;
    options.deliveryMode = PHVideoRequestOptionsDeliveryModeFastFormat;
...
- (void)exportAssetWithFallback:(AVAsset*)asset completion:(void(^)(NSString *outputPath))completion {
    NSString *outputFileName = [WKFileLocationHelper genFilenameWithExt:@"mp4"];
    NSString *outputPath = [WKFileLocationHelper filepathForVideo:outputFileName];
    ...
    [session exportAsynchronouslyWithCompletionHandler:^(void) {
        if (session.status == AVAssetExportSessionStatusCompleted) {
            ...
            [[NSFileManager defaultManager] removeItemAtPath:outputPath error:nil];
            NSString *fallbackFileName = [WKFileLocationHelper genFilenameWithExt:@"mp4"];
            NSString *fallbackPath = [WKFileLocationHelper filepathForVideo:fallbackFileName];
            AVAssetExportSession *fallbackSession = [[AVAssetExportSession alloc] initWithAsset:asset
                                                                                     presetName:AVAssetExportPresetPassthrough];
            fallbackSession.outputURL = [NSURL fileURLWithPath:fallbackPath];
            fallbackSession.outputFileType = AVFileTypeMPEG4;
            fallbackSession.shouldOptimizeForNetworkUse = YES;
            [fallbackSession exportAsynchronouslyWithCompletionHandler:^(void) {
