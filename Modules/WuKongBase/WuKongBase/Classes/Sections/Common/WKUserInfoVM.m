...
@property (nonatomic, copy) NSString *joinGroupInviteUID;
@property (nonatomic, copy) NSString *joinGroupInviteName;
@property (nonatomic, copy) NSString *joinGroupTime;
@property (nonatomic, copy) NSString *groupMemberInviteUID;
@property (nonatomic, copy) NSString *groupMemberCreatedAt;
...
- (void)loadPersonChannelInfo:(NSString *)uid completion:(channelInfoCompletion)completion {
    ...
    self.uid = uid;
    self.contextDict[@"join_group_invite_uid"] = @"";
    self.contextDict[@"join_group_invite_name"] = @"";
    self.contextDict[@"join_group_time"] = @"";
    self.contextDict[@"group_member_invite_uid"] = @"";
    self.contextDict[@"group_member_created_at"] = @"";
    ...
    [self requestUserDetail:uid].then(^(UserModel *user){
        ...
        weakSelf.contextDict[@"join_group_invite_uid"] = user.joinGroupInviteUID ?: @"";
        weakSelf.contextDict[@"join_group_invite_name"] = user.joinGroupInviteName ?: @"";
        weakSelf.contextDict[@"join_group_time"] = user.joinGroupTime ?: @"";
        weakSelf.contextDict[@"group_member_invite_uid"] = user.groupMemberInviteUID ?: @"";
        weakSelf.contextDict[@"group_member_created_at"] = user.groupMemberCreatedAt ?: @"";
...
-(void) initItems {
    ...
    [[WKApp shared] setMethod:@"user.info.inviteInfo" handler:^id _Nullable(id  _Nonnull param) {
        ...
        NSDictionary *context = param[@"context"];
        NSString *inviteUid = context[@"join_group_invite_uid"];
        NSString *inviteName = context[@"join_group_invite_name"];
        NSString *createdAt = context[@"join_group_time"];
        if ((inviteUid.length == 0 || createdAt.length == 0) && [context isKindOfClass:[NSDictionary class]]) {
            inviteUid = context[@"group_member_invite_uid"];
            createdAt = context[@"group_member_created_at"];
        }
        if (inviteUid.length == 0 || createdAt.length == 0) {
            return nil;
        }
        ...
        return  @{
            ...
            @"items":@[
                    @{
                        ...
                        @"value": [NSString stringWithFormat:@"%@ %@邀请入群",createdAt,inviteName],
                    },
            ],
        };
...
+ (UserModel *)fromMap:(NSDictionary *)dictory type:(ModelMapType)type {
    ...
    u.joinGroupInviteUID = [dictory objectForKey:@"join_group_invite_uid"] ?: @"";
    u.joinGroupInviteName = [dictory objectForKey:@"join_group_invite_name"] ?: @"";
    u.joinGroupTime = [dictory objectForKey:@"join_group_time"] ?: @"";
    NSDictionary *groupMember = [dictory objectForKey:@"group_member"];
    if([groupMember isKindOfClass:[NSDictionary class]]) {
        u.groupMemberInviteUID = groupMember[@"invite_uid"] ?: @"";
        u.groupMemberCreatedAt = groupMember[@"created_at"] ?: @"";
    } else {
        u.groupMemberInviteUID = @"";
        u.groupMemberCreatedAt = @"";
    }
    return u;
}
