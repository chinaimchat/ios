...
- (NSString *)onlineStatusTip:(WKChannelInfo *)channelInfo {
    ...
    NSString *onlineDeviceName = [self peerOnlineDeviceDisplayName:channelInfo.deviceFlag];
...
/// 对方会话/联系人等场景：不展示 Web/手机/PC 前缀时返回空串（仍由上层拼「在线」类文案）
- (NSString *)peerOnlineDeviceDisplayName:(WKDeviceFlagEnum)deviceFlag {
    if (deviceFlag == WKDeviceFlagEnumHidden) {
        return @"";
    }
    if ([WKApp shared].remoteConfig.showDeviceOnlineOn == 0) {
        return @"";
    }
    return [self deviceName:deviceFlag];
}
