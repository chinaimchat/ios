 ...
 *
 * iOS flow:
 * - WKWorkplaceVC open web: set store.pendingDismiss = YES, store.urlString/icon
 * - WKWebViewVC viewWillDisappear: if pendingDismiss=YES, set shouldShowBubble=YES
 * - WKWorkplaceVC viewWillAppear: consume shouldShowBubble and show bubble UI
 */
...
// "pendingDismiss" means the current WKWebViewVC was opened from Workplace and
// we should show bubble after user returns.
@property(nonatomic, assign) BOOL pendingDismiss;

// "shouldShowBubble" is consumed by WKWorkplaceVC to display the bubble.
...
@implementation WKWorkplaceWebBubbleStore

+ (instancetype)shared {
    static WKWorkplaceWebBubbleStore *store = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        store = [WKWorkplaceWebBubbleStore new];
        store.pendingDismiss = NO;
...
- (void)clear {
    ...
    self.icon = nil;
    self.webViewVC = nil;
    self.pendingDismiss = NO;
    self.shouldShowBubble = NO;
}
...
- (void)reloadFromWorkplaceDiscoverURLString:(NSString *)urlString {
    ...
    if (url && ![url hasPrefix:@"http"] && ![url hasPrefix:@"HTTP"]) {
...
- (void)viewDidLoad {
    ...
    if (self.skipInitialReload && self.webView.URL.absoluteString.length > 0) {
...
//        preferences.minimumFontSize = 40.0;
        ...
        _webView.scrollView.maximumZoomScale = 1.0f;
        _webView.scrollView.minimumZoomScale = 1.0f;
        _webView.scrollView.pinchGestureRecognizer.enabled = NO;
        [_webView addObserver:self forKeyPath:@"estimatedProgress" options:NSKeyValueObservingOptionNew context:nil];
        [self addUserScript:_webView];
        [self syncWebViewChromeColors];
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context
{
    if (object == self.webView && [keyPath isEqualToString:@"estimatedProgress"]) {
        ...
        if (store.webViewVC == self && self.webView.URL.absoluteString.length > 0) {
            store.urlString = self.webView.URL.absoluteString;
            self.currentUrl = self.webView.URL;
        }
        [self checkGoAndGobackBtn];
    } else {
...
- (void)viewWillDisappear:(BOOL)animated {
    ...
    WKWorkplaceWebBubbleStore *store = [WKWorkplaceWebBubbleStore shared];
    if (store.pendingDismiss && (self.isMovingFromParentViewController || self.isBeingDismissed)) {
        // Returning to Workplace should behave like minimizing a browser window:
        // keep the same WKWebViewVC instance and latest URL for bubble restore.
        store.webViewVC = self;
        if (self.currentUrl.absoluteString.length > 0) {
            store.urlString = self.currentUrl.absoluteString;
        } else if (self.webView.URL.absoluteString.length > 0) {
            store.urlString = self.webView.URL.absoluteString;
        }
        store.shouldShowBubble = YES;
        store.pendingDismiss = NO;
    }
}
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler{
    ...
    if([reqUrl hasPrefix:@"http"] && ![self.url.host containsString:@"pgyer.com"]) { // pgyper 特殊处理下
        ...
        WKWorkplaceWebBubbleStore *store = [WKWorkplaceWebBubbleStore shared];
        if (store.webViewVC == self && self.currentUrl.absoluteString.length > 0) {
            store.urlString = self.currentUrl.absoluteString;
        }
...
- (void)addUserScript:(WKWebView *)webView {
    NSString *disableZoomJs = @"(function(){"
    ...
    "})();";
    WKUserScript *zoomScript = [[WKUserScript alloc] initWithSource:disableZoomJs
...
/// 与 App 深色/浅色一致，便于页面内 CSS `prefers-color-scheme` 与 WKContentModeRecommended 对齐。
- (void)syncWebViewChromeColors {
    if (!_webView) {
        return;
    }
    UIColor *bg = [WKApp shared].config.backgroundColor;
    _webView.backgroundColor = bg;
    _webView.scrollView.backgroundColor = bg;
}
