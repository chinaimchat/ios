...
/// 与 Android {@code WKImageProvider#cacheImageIfNeeded} 一致：避免同一 {@code clientMsgNo} 重复落盘。
static NSMutableSet<NSString *> *wkImageDiskCacheInFlight(void) {
...
- (void)refresh:(WKMessageModel *)model {
    ...
    if(orgData) {
        ...
        } else if(!model.content.flame && imageContent.message && imageContent.remoteUrl.length > 0) {
            /// 已下载到 IM 消息目录（与 Android 写入 {@code localPath} 后再次进入会话一致）。
            NSString *diskPath = imageContent.localPath;
            if ([[NSFileManager defaultManager] fileExistsAtPath:diskPath]) {
                NSDictionary *attr = [[NSFileManager defaultManager] attributesOfItemAtPath:diskPath error:nil];
                if ([attr fileSize] > 0) {
                    NSData *diskData = [NSData dataWithContentsOfFile:diskPath];
                    if (diskData.length > 0) {
                        [self setImageWithData:diskData];
                    }
                }
            }
            if (!self.imgView.image) {
                ...
                if (url) {
                    ...
                    [self.imgView lim_setImageWithURL:url options:SDWebImageProgressiveLoad | SDWebImageScaleDownLargeImages context:@{
                        ...
                        if (!model.content.flame && image && imageContent.message && imageContent.remoteUrl.length > 0) {
                            [weakSelf wk_persistDownloadedImage:image forImageContent:imageContent];
                        }
                    }];
                }
            }
...
/// 与 Android {@code WKImageProvider#cacheImageIfNeeded} 一致：网络图落盘到 {@link WKImageContent#localPath}，下次优先走本地。
- (void)wk_persistDownloadedImage:(UIImage *)image forImageContent:(WKImageContent *)content {
    if (!image || !content.message) {
        return;
    }
    NSString *key = content.message.clientMsgNo;
    if (key.length == 0) {
        return;
    }
    NSMutableSet *inFlight = wkImageDiskCacheInFlight();
