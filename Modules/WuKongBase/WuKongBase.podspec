...
#  s.resource  = 'WuKongBase/WuKongIMSDK-Framework/ios/WuKongIMSDK.framework/Versions/A/Resources/WuKongIMSDK.bundle'
  buglypro_xcframework = File.join(__dir__, 'WuKongBase', 'BuglyPro.xcframework')
  bugly_xcframework = File.join(__dir__, 'WuKongBase', 'Bugly.xcframework')

  if File.exist?(buglypro_xcframework)
    s.vendored_frameworks = 'WuKongBase/BuglyPro.xcframework'
  elsif File.exist?(bugly_xcframework)
    s.vendored_frameworks = 'WuKongBase/Bugly.xcframework'
    Pod::UI.warn('WuKongBase: using legacy Bugly.xcframework. Prefer BuglyPro.xcframework for Apple Silicon simulator support.')
  else
    s.vendored_frameworks = 'WuKongBase/Bugly.framework'
    Pod::UI.warn('WuKongBase: BuglyPro.xcframework/Bugly.xcframework not found, fallback to Bugly.framework (simulator may fail).')
  end
...
#  s.dependency 'FLEX'
  ...
  s.dependency 'YYImage/libwebp','~> 1.0.4'
