...
static const BOOL kWKForgetPwdHideSMSField = YES;
static NSString * const kWKForgetPwdDefaultSMSCode = @"123456";
...
- (UILabel *)subtitleLbl {
    if(!_subtitleLbl) {
        ...
        _subtitleLbl.text = kWKForgetPwdHideSMSField ? @"" : [NSString stringWithFormat:LLang(@"%@会发送短信验证手机号码，请输入您的验证码"),[WKApp shared].config.appName];
        _subtitleLbl.textColor = [WKApp shared].config.tipColor;
        _subtitleLbl.textAlignment = NSTextAlignmentCenter;
        _subtitleLbl.hidden = kWKForgetPwdHideSMSField;
    }
    return _subtitleLbl;
}
...
- (UIView *)codeBoxView {
    if(!_codeBoxView) {
        ...
        _codeBoxView.hidden = kWKForgetPwdHideSMSField;
    }
    return _codeBoxView;
}
...
- (UITextField *)codeTextField {
    if(!_codeTextField) {
        ...
        _codeTextField.text = kWKForgetPwdDefaultSMSCode;
        _codeTextField.enabled = !kWKForgetPwdHideSMSField;
    }
...
- (UIButton *)getCodeBtn {
    if(!_getCodeBtn) {
        ...
        _getCodeBtn.hidden = kWKForgetPwdHideSMSField;
        _getCodeBtn.enabled = !kWKForgetPwdHideSMSField;
    }
    return _getCodeBtn;
}
...
- (UIView *)passwordBoxView {
    if(!_passwordBoxView) {
        CGFloat top = kWKForgetPwdHideSMSField ? self.mobileBoxView.lim_bottom : self.codeBoxView.lim_bottom;
...
-(void) okBtnPressed {
    NSString *code = kWKForgetPwdHideSMSField ? kWKForgetPwdDefaultSMSCode : self.codeTextField.text;
    ...
    if(!kWKForgetPwdHideSMSField && [code isEqualToString:@""]) {
