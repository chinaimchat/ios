...
+(void) handleLoginData:(WKLoginResp*)resp isSave:(BOOL)isSave{
    [WKApp shared].loginInfo.token = resp.token ? [NSString stringWithFormat:@"%@", resp.token] : @"";
    if(resp.imToken && [NSString stringWithFormat:@"%@", resp.imToken].length > 0) {
        [WKApp shared].loginInfo.imToken = [NSString stringWithFormat:@"%@", resp.imToken];
    }else {
        [WKApp shared].loginInfo.imToken = [WKApp shared].loginInfo.token;
    }
    [WKApp shared].loginInfo.uid = resp.uid ? [NSString stringWithFormat:@"%@", resp.uid] : @"";
