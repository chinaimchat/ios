...
/// 与 Android 一致：不展示短信验证码输入框，注册时固定传默认码
static const BOOL kWKRegisterHideSMSField = YES;
static NSString * const kWKRegisterDefaultSMSCode = @"123456";
...
- (UIView *)passwordBoxView {
    if(!_passwordBoxView) {
        CGFloat top = kWKRegisterHideSMSField ? (self.mobileBoxView.lim_bottom + 20.0f) : (self.codeBoxView.lim_bottom + 20.0f);
        _passwordBoxView = [[UIView alloc] initWithFrame:CGRectMake(0, top, WKScreenWidth, self.mobileBoxView.lim_height)];
       // [_passwordBoxView setBackgroundColor:[UIColor grayColor]];
    }
...
- (UITextField *)confirmPasswordTextField {
    if(!_confirmPasswordTextField) {
        ...
        [_confirmPasswordTextField setPlaceholder:LLang(@"请再次输入登录密码")];
...
- (UIView *)inviteCodeBoxView {
    if(!_inviteCodeBoxView) {
        _inviteCodeBoxView = [[UIView alloc] initWithFrame:CGRectMake(0, self.confirmPasswordBoxView.lim_bottom+20.0f, WKScreenWidth, self.passwordBoxView.lim_height)];
    }
...
-(void) registerBtnPressed {
    NSString *code = kWKRegisterHideSMSField ? kWKRegisterDefaultSMSCode : self.codeTextField.text;
    ...
    if(!confirmPassword || [confirmPassword isEqualToString:@""]) {
        [self.view showHUDWithHide:LLang(@"请再次输入登录密码")];
        return;
    }
